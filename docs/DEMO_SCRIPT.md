...
| 0:38–1:00 | Show `request_applicant_fact` appear because the audience fact is blank. Call it and hold briefly on its immediate `awaiting_human` result and page-owned question, then answer through native **Share answer with agent**. Show the tool list again after it disappears. | One blocker is a fact only I can truthfully provide. The page exposes one contextual tool. It opens the question and returns control immediately; I answer through the native form. The agent then re-reads the same live page, and the tool disappears. |
